...
var _default = async (request, response, next) => {
  ...
  if (request.headers.authorization) {
    ...
    if (bearerToken === 'Bearer') {
      try {
        const blacklistedToken = await _redis.default.verifyBlacklistForToken(token);
        if (blacklistedToken) {
          if (language === 'en-US') next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorUnauthorized, 'Token invalidated by logout.'));else next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorUnauthorized, 'Token invalidado por logout.'));
        }
    ...
    if (language === 'en-US') next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorUnauthorized, 'Invalid bearer token.'));else next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorUnauthorized, 'Bearer token inválido.'));
  }
  if (language === 'en-US') next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorBadRequest, 'Authorization header is not present.'));else next((0, _http.responseError)(response, {}, _statusCodeEnum.default.ClientErrorBadRequest, 'Cabeçalho de autorização não está presente.'));
};
