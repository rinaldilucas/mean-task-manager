...
exports.findOneByEmail = async (request, response) => {
    const [data, error] = await httpHandler.parser(request, response, Model.findOne({ email: request.params.email }));
    if (!!error) return;
...
exports.register = async (request, response) => {
    ...
    const [salt] = await httpHandler.parser(request, response, bcrypt.genSalt(+process.env.SALT_RONDS));
    const [hashPass] = await httpHandler.parser(request, response, bcrypt.hash(request.body.password, salt));
    ...
    const [document, documentError] = await httpHandler.parser(request, response, Model.findOne({ email: request.body.email }));
    if (!!documentError) return;
    if (document)
    ...
    const [data, error] = await httpHandler.parser(request, response, newUser.save());
    if (!!error) return;
    if (!data)
...
exports.authenticate = async (request, response) => {
    ...
    const [validation, validationError] = await httpHandler.parser(request, response, bcrypt.compare(request.body.password, document.password));
...
exports.logout = async (request, response) => {
    ...
    const [, addToBlacklistError] = await httpHandler.parser(request, response, blacklistHandler.add(token));
...
exports.changePassword = async (request, response) => {
    ...
    const [data, error] = await httpHandler.parser(request, response, Model.updateOne({ _id: request.body._id }, newBody, { new: true }));
    if (!!error) return;
    if (!data || data.n === 0)
        if (language == 'en-US') return httpHandler.error(response, {}, StatusCode.ClientErrorBadRequest, `Error updating document with id ${request.body._id}.`);
        else return httpHandler.error(response, {}, StatusCode.ClientErrorBadRequest, `Erro ao atualizar documento de id ${request.body._id}. Nome do documento: {${Model.modelName}}.`);
